...
/*   Updated: 2019/01/15 11:30:14 by qrosa            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void 			PresidentialPardonForm::execute(Bureaucrat const &bur) const
{
	...
	{
			std::cout	<< "Bureaucrat " << bur.getName() << " (" << bur.getGrade()\
				<< ") executes a " << this->getName() << " Form (s_grade " << this->getGradeToSign()\
				<< this->getGradeToExec() << ") targeted on " << this->_target\
				<< " (" << this->signed_str(*this) << ")" << std::endl;
